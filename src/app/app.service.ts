...
export class AppService {
  ...
  colCnt: number = 30;
	...
	private _headerSize: ScreenSize = {
		h: 56,
	...
	private _scrnMetrics$: ReplaySubject<ScreenSize> = new ReplaySubject(1);
	private _availableContentMetrics$: ReplaySubject<ScreenSize> = new ReplaySubject(1);
	...
	set scrnMetrics(scrnSize: ScreenSize) {
		...
		this._availableContentSize = <ScreenSize>{
			h: scrnSize.h - this._headerSize.h,
			w: scrnSize.w
		};
		this._scrnMetrics$.next(scrnSize);
	...
	get scrnMetricsObservable(): Observable<ScreenSize> {
		return this._scrnMetrics$.asObservable();
	}
	...
	public setGridRem() {
		...
		this._gridMetrics$.next({h: this._colSize, w: this._gridSize, ch: (this.cellCnt +1) * this.cellHeight});
	}

	private getStyleTextNode = (v: number) => `html{ font-size: ${v}px !important; }`;
   ...
   */
	public getCustomFontSize(): number {
		...
		const r: number = h > 0 ? Math.sqrt((c/g) * Math.pow(h, 2) / (Math.pow(3.25, 2) * (c * g))) : this._fontSize; // gridSize is always greater than colSize.
	...
	private handleRootFontSize(custFontSize: number = -1) {
		...
		resteEl.textContent = this.getStyleTextNode(custFontSize > 0 ? custFontSize : this._fontSize);
	}
}
